...
def fluxMap(xp_cm, yp_cm, width_cm, bin_um):
    ...
    TODO: In documentation, specify which axis is X axis, which is Y axis in flux2D
    ...
    flux2D = H.T # Perform a transpose (flip x/y axis) on the histogram so that it plots correctly
...
def fluxPlot(outfn, flux2D, bin_um):
    ...
    Written by Scott Feister 2017-08-03
    TODO: Check if I have flipped the X and Y axes.
    """
    fig = plt.figure(figsize=(6,5))
    ax = fig.add_subplot(111)
    
    xmax = flux2D.shape[0] * bin_um*1.0e-4 # X/ length of detector in cm
    ymax = flux2D.shape[1] * bin_um*1.0e-4 # Y/ width of detector in cm
